<template>
  <div id="app">
    ...
    <div class="server-ctn">
      ...
      <input id="server-input" v-model="serverURL" placeholder="Enter server URL" />
    
      <button @click.prevent="applyServerSettings">Apply</button>
      <button @click="createParticles" class="explosion-btn">Click me!</button>
...
/* Buttons */
/* Ensure the basics of the button are set for positioning the particles */
.explosion-btn {
  position: relative;
  overflow: hidden; /* Ensure particles are clipped to the button bounds */
}

/* Particle styles and keyframes */
.particle {
  position: absolute;
  border-radius: 50%; /* Keep it round */
  background-color: gold; /* Particle color, you might want to randomize this as well */
...
@keyframes explode {
  ...
  100% {
    /* Use larger values for scale to make the explosion bigger */
...
button {
  ...
  color: #fff;
  padding: 0.5rem 1rem;
  border: 3px solid #457b9d; /* Customizable border */
  border-radius: 5px; /* Rounded corners */
  cursor: pointer;
  transition: all 0.3s ease-in-out; /* Smooth transition for hover effects */
  position: relative; /* For the particle effect positioning */
  overflow: hidden; /* Ensures particles do not overflow the button boundaries */
}

button:hover {
  background-color: #457b9d;
  border-color: #1d3557; /* Change border color on hover for a nice effect */
